#!/usr/bin/env python
"""
Production-Ready Options Trading Bot with Dynamic Options Screening, Iron Condor Strategy & Advanced Risk Management

This bot uses Alpaca’s options contracts endpoint (via a dedicated REST client with api_version="v2")
to dynamically retrieve options chain data, and then uses a transformer-based deep Q-network (RL agent)
to decide whether to trade an option. It now supports an iron condor strategy, which involves selling an OTM call 
and put and buying a further OTM call and put to cap risk. Option orders are submitted using the selected contracts.
A dedicated websocket subscriber receives real-time option and bar data.
...
import os, json, math, random, logging, asyncio, pickle, requests
...
class AlpacaTrader:
    ...
    async def submit_order(self, **kwargs):
        # Remove the unsupported "time_in_force" if present.
    ...
    # Retrieve options contracts using a direct REST call with proper filters.
...
def select_iron_condor_legs(contracts: list, underlying_price: float, spread_width: float) -> Dict[str, Dict]:
    calls = [c for c in contracts if c.get("type", "").lower() == "call"]
    puts  = [c for c in contracts if c.get("type", "").lower() == "put"]
    if not calls or not puts:
        raise ValueError("Options chain does not contain both calls and puts.")
    calls.sort(key=lambda c: float(c["strike_price"]))
    puts.sort(key=lambda c: float(c["strike_price"]))
    short_call = next((c for c in calls if float(c["strike_price"]) >= underlying_price), calls[-1])
    short_put = next((p for p in reversed(puts) if float(p["strike_price"]) <= underlying_price), puts[0])
    sc_strike = float(short_call["strike_price"])
    long_call = next((c for c in calls if float(c["strike_price"]) >= sc_strike + spread_width), calls[-1])
    sp_strike = float(short_put["strike_price"])
    long_put = next((p for p in reversed(puts) if float(p["strike_price"]) <= sp_strike - spread_width), puts[0])
    logger.info(f"Iron Condor Legs for underlying at {underlying_price}: Short Call @ {short_call['strike_price']}, "
                f"Long Call @ {long_call['strike_price']}, Short Put @ {short_put['strike_price']}, "
                f"Long Put @ {long_put['strike_price']}")
    return {
...
async def execute_iron_condor(trader: AlpacaTrader, symbol: str, expiration_date: str, spread_width: float) -> List:
    ...
    if not contracts:
        ...
        return []
    try:
        underlying_price = await trader.get_latest_price(symbol)
    except Exception as e:
    ...
    legs = select_iron_condor_legs(contracts, underlying_price, spread_width)
    orders = []
    order_specs = [
...
# process_options remains for single-leg execution if needed.
...
# Modified process_symbol: based on RL action, either execute single-leg or iron condor.
async def process_symbol(symbol: str, rl_trader, trader: AlpacaTrader, logger: logging.Logger):
    ...
    # Actions: 0 = Hold, 1 = Buy Call, 2 = Buy Put, 3 = Execute Iron Condor.
    if rl_action == 0:
